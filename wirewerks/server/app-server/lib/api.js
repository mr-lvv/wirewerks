...
class Api {
	constructor(app, config) {
		...
		client.get('/sections', (req, res) => {
			Products.find({}, 'type description number').exec(function (err, sections) {
				if (err) {
		...
		client.get('/section/:number', (req, res) => {
			Products.findOne({ "number": req.params.number }).exec(function (err, section) {
				if (err || !section) {
					res.status(200).send({ 'notFound': true });
					return;
				}
		...
		client.post('/add/:sectionNumber', (req, res) => {
			Products.findOne({ "number": req.params.sectionNumber }).then(function (section) {
